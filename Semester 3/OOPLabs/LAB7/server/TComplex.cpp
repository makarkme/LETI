...
Complex pow(Complex z, double n) {
    double argZ;
    if (z.getRe() > 0)
        argZ = atan(z.getIm() / z.getRe());
    else if (z.getRe() < 0 && z.getIm() >= 0)
        argZ = 3.14 - atan(z.getIm() / z.getRe());
    else if (z.getRe() < 0 && z.getIm() < 0)
        argZ = -3.14 + atan(z.getIm() / z.getRe());
    else if (z.getRe() == 0 && z.getIm() > 0)
        argZ = 3.14 / 2;
    ...
    double absZ = std::sqrt(z.getRe()*z.getRe() + z.getIm()*z.getIm());
    return Complex(pow(absZ, n) * cos(argZ * n),
                   pow(absZ, n) * sin(argZ * n));
}
...
Complex Complex::operator/(long long a) const{
    Complex t{};
    t.re = re/a;
    t.im = im/a;
    return t;
