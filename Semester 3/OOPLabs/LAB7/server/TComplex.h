...
public:
    ...
    Complex operator/(long long) const; // перегрузка оператора деления на long long
    ...
    void setRe(double r);
    void setIm(double i);
    friend Complex pow(Complex z, double n);
    ~Complex(); // desctructor
};

inline std::ostream& operator<<(std::ostream& os, const Complex& c) { // перегрузка оператора ввода
    os << "(" << c.getRe() << "+" << c.getIm() << "i)";
    return os;
}

inline std::istream& operator>>(std::istream& is, Complex& c) { // перегрузка оператора вывода
    double r, i;
    is >> r >> i;
    c.setRe(r);
    c.setIm(i);
